...
<template>
  <div class="rounded-2xl overflow-hidden border border-white/10 bg-white text-slate-900 shadow-xl">
    <div class="bg-gradient-to-r from-rose-600 to-orange-500 text-white px-5 py-3 flex items-center justify-between">
      <span class="font-bold tracking-tight">SoundWave Audio</span>
      <span class="text-xs bg-white/20 rounded-full px-2 py-0.5">⚡ Flash Sale</span>
    </div>
    ...
    <div class="p-5 sm:p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <Hotspot as="div" :found="is('fake-stock-counter')" @trigger="emit('found', 'fake-stock-counter')">
          <div class="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-center">
            <p class="text-xs text-amber-700 font-semibold">🔥 Only 2 left in stock!</p>
          </div>
        </Hotspot>
        ...
        <Hotspot as="div" :found="is('fake-social-proof')" @trigger="emit('found', 'fake-social-proof')">
          <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-center">
            <p class="text-xs text-slate-600 font-semibold">👀 14 people viewing this</p>
          </div>
        </Hotspot>
      </div>
      ...
      <div class="flex flex-col items-center gap-2 pt-1">
        <button
          v-if="!claimed"
          class="w-full rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 shadow-md transition"
          @click="claimDeal"
        >
          Claim This Deal Now
        </button>
        <div v-else class="w-full rounded-xl bg-slate-100 text-slate-500 text-sm text-center py-3">
          You clicked "Claim Deal" — but the timer never really runs out. 😬
        </div>
