  ...
  <section class="max-w-5xl mx-auto px-5 py-12">
    ...
    <div class="grid sm:grid-cols-3 gap-5">
      <RouterLink
        ...
        :key="s.id"
        :to="`/scenario/${s.id}`"
        class="group bg-judge-panel border border-white/10 hover:border-judge-accent/50 rounded-2xl p-5 flex flex-col transition"
      >
        ...
        <h2 class="font-bold text-lg leading-snug mb-2 group-hover:text-judge-accent transition">{{ s.title }}</h2>
        ...
        <div class="mt-4">
          <div class="flex items-center justify-between text-xs text-slate-500 mb-1">
            ...
            <span v-if="store.progress[s.id].completed" class="text-judge-accent2 font-semibold">✓ Done</span>
          </div>
          <div class="h-1.5 rounded-full bg-white/5 overflow-hidden">
            <div
              class="h-full bg-judge-accent rounded-full transition-all"
